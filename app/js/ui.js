...
function openSettings(){
  ...
  // Inject parent access button in child view — calls existing quickSettings() which opens the picker
  ...
  if(_spIsChild){
    if(!_pab){
      ...
      _pab.innerHTML='<button style="width:100%;padding:.4rem;background:rgba(255,255,255,.05);color:var(--tx2);border:1px solid rgba(255,255,255,.1);border-radius:9px;font-size:.73rem;font-weight:700;cursor:pointer;font-family:var(--fn);" onclick="document.getElementById(\'sp\').classList.remove(\'child-view\');closeSettings();setTimeout(quickSettings,150)">🔒 Parent Settings</button>';
