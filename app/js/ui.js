...
function switchToParentSettings(){
  // Use parent.js's _openParentSettings which handles PIN + profile switching
  if(typeof _openParentSettings === 'function'){
    closeSettings();
    setTimeout(_openParentSettings, 100);
  } else {
    // Fallback if parent.js not loaded
    if(D.parentPIN){
      var _e=prompt('Enter Parent PIN:'); if(_e===null) return;
      if(String(_e).trim()!==String(D.parentPIN)){ showToast('Incorrect PIN'); return; }
    }
