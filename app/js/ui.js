...
function openSettings(){
  ...
  // Show/hide parent-access button in child view
  var _pab=document.getElementById('sp-parent-access-btn');
  if(_spIsChild){
    if(!_pab){
      _pab=document.createElement('div');
      _pab.id='sp-parent-access-btn';
      _pab.style.cssText='padding:.5rem 1rem 0;';
      _pab.innerHTML='<button style="width:100%;padding:.4rem;background:rgba(255,255,255,.05);color:var(--tx2);border:1px solid rgba(255,255,255,.1);border-radius:9px;font-size:.73rem;font-weight:700;cursor:pointer;font-family:var(--fn);" onclick="switchToParentSettings()">🔒 Parent Settings</button>';
      var _spPb=document.getElementById('sp-pb');
      if(_spPb&&_spPb.parentNode) _spPb.parentNode.insertBefore(_pab,_spPb);
    }
    _pab.style.display='';
  } else {
...
function switchToParentSettings(){
  // Find parent profile
  var parentProf = (_profiles||[]).find(function(p){ return p.isParent===true; })
    || (_profiles||[]).find(function(p){ return p.isParent!==false; });

  if(!parentProf){ openSettings(); return; }

  // Check PIN from parent's own saved data
  var parentData = (parentProf.data) ? parentProf.data : {};
  var pin = parentData.chorePin || parentData.parentPIN || D.chorePin || D.parentPIN || '';

  function doOpen(){
    // Save current child data back to child profile
    var childProf = (_profiles||[]).find(function(p){ return p.id===_activeProfileId; });
    if(childProf && childProf.id !== parentProf.id){
      childProf.data = JSON.parse(JSON.stringify(D));
    }
    // Switch active profile to parent
    _activeProfileId = parentProf.id;
    localStorage.setItem('ylcc_active_profile', _activeProfileId||'');
    // Load parent data into D
    if(parentProf.data && Object.keys(parentProf.data).length){
      for(var k in D){ if(D.hasOwnProperty(k)) delete D[k]; }
      Object.assign(D, parentProf.data);
    }
    // Open settings — now _activeProfileId is parent so child-view won't apply
    openSettings();
  }

  if(pin && !parentData.parentPinDisabled && !D.parentPinDisabled){
    if(typeof showPinModal === 'function'){
      closeSettings();
      showPinModal('Parent Settings','Enter your parent PIN',
        (parentData.name||D.name||'P').charAt(0).toUpperCase(), 'enter', doOpen);
    } else {
      var entered = prompt('Enter Parent PIN:');
      if(entered === null) return;
      if(String(entered).trim() !== String(pin)){ showToast('Incorrect PIN'); return; }
      closeSettings();
      doOpen();
    }
  } else {
