...
function toggleParentMode(){
  ...
  if(_parentMode){
    _parentMode=false;
    document.getElementById('choreSetupArea').style.display='none';
    document.getElementById('parentModeBtn').innerHTML='🔒 Parent Mode';
    document.getElementById('parentModeBtn').classList.remove('bp');
    document.getElementById('parentModeBtn').classList.add('bgh');
    renderChores();
    return;
  }
  _requirePin('Parent Mode','Enter your 6-digit parent PIN',(D.name||'P').charAt(0).toUpperCase(),()=>{
    _parentMode=true;
    document.getElementById('choreSetupArea').style.display='block';
    document.getElementById('parentModeBtn').innerHTML='🔓 Parent Mode ON';
    document.getElementById('parentModeBtn').classList.add('bp');
    document.getElementById('parentModeBtn').classList.remove('bgh');
    renderChores();
  });
}
...
function removeChore(id){
  D.chores = (Array.isArray(D.chores)?D.chores:[]).filter(c=>c.id!==id); save(); renderChores();
}
