...
function quickSettings(){
  // If only one profile (no children added), go straight to settings
  var children=(_profiles||[]).filter(function(p){return p.isParent===false;});
  if(!children.length){ openSettings(); return; }
  // Multiple profiles — show picker so parent can choose
  openSettingsPicker();
}
...
<!-- Google Translate widget loader — required for language picker -->
