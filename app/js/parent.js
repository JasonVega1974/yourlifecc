...
function _openParentSettings(){
  ...
  // Always find the parent profile first
  var parentProf = _profiles.find(function(p){ return p.isParent===true; })
    || _profiles.find(function(p){ return p.isParent!==false; })
    || null;

  // Read PIN from parent profile's saved data, not current D (which may be child's)
  ...
  if(pin && !D.parentPinDisabled && !(parentData.parentPinDisabled)){
    showPinModal('Parent Settings','Enter your parent PIN',(parentData.name||D.name||'P').charAt(0).toUpperCase(),'enter', doSwitch);
    return;
  }
  doSwitch();
}
