...
// ── RETIRED 2026-06-08 (Email Bundle PR 0) ───────────────────
// emailWeeklySummary() called /api/send-email with a `to` field that
// the endpoint silently ignored — every "weekly summary email" was
// being routed to info@kingdom-creatives.com (Jason's inbox), not
// the parent. Track 1 of the email bundle replaces this with a real
// Sunday-7pm digest backed by /api/cron/weekly-digest. Function
// preserved as a no-op so the existing UI button + the
// checkAutoWeeklyEmail timer don't ReferenceError until they're
// rewired. Both surface "Coming soon" copy.
async function emailWeeklySummary(childId){
  if(typeof showToast === 'function'){
    showToast('Weekly digest moves to Sunday 7pm in Track 1 — coming soon');
  }
  return;
}

// Original body kept for Track 1 reference. The new digest reads from
// D.activityLog (FAF Inc 2 source of truth) instead of walking the
// per-tab arrays this version did.
async function _emailWeeklySummary_RETIRED(childId){
...
<div style="max-width:540px;margin:0 auto;background:#161b27;border-radius:14px;overflow:hidden;border:1px solid rgba(56,189,248,.15);">
  ...
  <div style="padding:20px 24px;">
    ...
    <div style="margin-top:16px;text-align:center;font-size:11px;color:#334155;padding-top:12px;border-top:1px solid #1e293b;">
      This report was generated by <span style="color:#38bdf8;">YourLife CC</span> · <a href="https://yourlifecc.com" style="color:#38bdf8;">yourlifecc.com</a><br>
...
// ── AUTO MONDAY EMAIL — RETIRED 2026-06-08 (Email Bundle PR 0) ─
// The original Monday auto-email called emailWeeklySummary() which
// silently misrouted every send to info@kingdom-creatives.com (see
// retirement note on emailWeeklySummary). Track 1 replaces this
// with a real Sunday-7pm digest driven by /api/cron/weekly-digest
// (Vercel Cron), keyed by D.emailPrefs.timezoneOffsetMin so the
// email lands in the parent's inbox at their local 7pm. Stub is
// preserved so the post-load setTimeout doesn't ReferenceError.
function checkAutoWeeklyEmail(){
  // No-op — see PR 0 retirement note. Track 1 takes over.
  return;
}
