...
function initDriving(){ renderDriverChecklist(); }
...
function _pinKey(k){
  ...
  _pinEntry += String(k);
  _pinUpdateDots();
  // Auto-submit when entry matches the correct PIN length (4 or 6 digit support)
  var correct = D.chorePin || D.parentPIN || '';
  // Also check parent profile data if available
  var parentProf = (_profiles||[]).find(function(p){ return p.isParent===true || p.isParent!==false; });
  var parentData = (parentProf && parentProf.data) ? parentProf.data : null;
  if(parentData) correct = parentData.chorePin || parentData.parentPIN || correct;
  var targetLen = correct.length || 6;
  if(_pinEntry.length >= targetLen) setTimeout(_pinSubmit, 200);
}
...
function _pinSubmit(){
  if(_pinMode === 'enter'){
    // Check against current D AND parent profile data
    var correct = D.chorePin || D.parentPIN || '';
...
function openSettingsPicker(){
  ...
  // Find explicit parent profile — never fall back to active child profile
  var parent = _profiles.find(function(p){ return p.isParent===true; })
    || _profiles.find(function(p){ return p.isParent!==false; })
    || (_profiles.length ? _profiles[0] : null);
  var children = _profiles.filter(function(p){ return p.isParent===false; });

  var html='';
  if(parent){
    var parentLabel = parent.isParent===true ? (parent.name && parent.name!=='' ? parent.name : 'Parent') : 'Parent';
    html+='<button class="spm-btn parent" onclick="closeSettingsPicker();_openParentSettings()">'
      +'<span style="font-size:1.2rem;">👨‍👩‍👧</span><div><div>'+parentLabel+' (You)</div>'
...
function _openParentSettings(){
  ...
  // Read PIN — try parent profile data first, fall back to current D
  var parentData = (parentProf && parentProf.data) ? parentProf.data : D;
  var pin = parentData.chorePin || parentData.parentPIN || D.chorePin || D.parentPIN;

  function doOpen(){
    // Just update _activeProfileId — don't touch D or trigger full re-render
    if(parentProf) {
      _activeProfileId = parentProf.id;
      localStorage.setItem('ylcc_active_profile', _activeProfileId||'');
    }
    // Force remove child-view immediately so sp-pb sections become visible
    var spEl = document.getElementById('sp');
    if(spEl) spEl.classList.remove('child-view');
    openSettings();
  }

  if(pin && !D.parentPinDisabled && !(parentData.parentPinDisabled)){
