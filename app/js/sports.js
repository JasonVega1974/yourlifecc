...
function createSportModal(){
  ...
  m.innerHTML = `
    <div style="background:var(--card);border-radius:20px;max-width:500px;width:100%;max-height:80vh;overflow-y:auto;padding:1.4rem;">
...
function renderMySports(){
  ...
  container.innerHTML = sports.map(s=>{
    ...
    const latestSeason = s.seasons && s.seasons.length ? s.seasons[s.seasons.length-1] : null;
    return `
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:16px;margin-bottom:.8rem;overflow:hidden;">
      <!-- Header -->
      <div style="display:flex;align-items:center;gap:.7rem;padding:.9rem 1rem;border-bottom:1px solid rgba(255,255,255,.05);">
        <div style="font-size:1.8rem;">${s.emoji}</div>
        <div style="flex:1;">
          <div style="font-size:.92rem;font-weight:900;color:var(--tx);">${s.name}</div>
          <div style="font-size:.72rem;color:var(--tx2);">${[s.team,s.position,s.level,s.season,s.year].filter(Boolean).join(' · ')}</div>
        </div>
        <div style="display:flex;gap:.4rem;">
          <button onclick="openSportEditor(${s.id})" style="background:rgba(var(--c-rgb),.15);border:1px solid rgba(var(--c-rgb),.3);border-radius:8px;padding:.3rem .7rem;color:var(--c);font-size:.72rem;cursor:pointer;font-weight:700;">✏️ Edit</button>
          <button onclick="deleteMySport(${s.id})" style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:8px;padding:.3rem .6rem;color:#f87171;font-size:.72rem;cursor:pointer;">✕</button>
        </div>
      </div>

      <!-- Season Stats preview -->
      ${latestSeason ? `
      <div style="padding:.7rem 1rem;border-bottom:1px solid rgba(255,255,255,.05);">
        <div style="font-size:.65rem;text-transform:uppercase;letter-spacing:1px;color:var(--tx2);margin-bottom:.4rem;font-weight:700;">📊 ${latestSeason.seasonLabel || 'Latest Season'}</div>
        <div style="display:flex;flex-wrap:wrap;gap:.4rem;">
          ${Object.entries(latestSeason.stats||{}).filter(([k,v])=>v).map(([k,v])=>{
            const field = data && data.statFields ? data.statFields.find(f=>f.key===k) : null;
            return `<div style="background:rgba(255,255,255,.04);border-radius:8px;padding:.3rem .6rem;text-align:center;min-width:50px;">
              <div style="font-size:.85rem;font-weight:900;color:var(--c);">${v}</div>
              <div style="font-size:.6rem;color:var(--tx2);">${field?field.label:k}</div>
            </div>`;
          }).join('')}
        </div>
      </div>` : ''}
      ...
      <div style="padding:.6rem 1rem;">
        <button onclick="openSeasonEditor(${s.id})" style="background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);border-radius:8px;padding:.35rem .8rem;color:#22c55e;font-size:.72rem;cursor:pointer;font-weight:700;width:100%;">
          📈 ${s.seasons&&s.seasons.length ? 'Add / View Season Stats' : 'Add Season Statistics'}
        </button>
      </div>
    </div>`;
  }).join('');
...
function createSportEditorModal(){
  ...
  m.innerHTML=`
    <div style="background:var(--card);border-radius:20px;max-width:460px;width:100%;max-height:85vh;overflow-y:auto;padding:1.4rem;">
...
function createSeasonEditorModal(){
  ...
  m.style.cssText='display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;align-items:center;justify-content:center;padding:1rem;';
  m.innerHTML=`
    <div style="background:var(--card);border-radius:20px;max-width:480px;width:100%;max-height:85vh;overflow-y:auto;padding:1.4rem;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.8rem;">
