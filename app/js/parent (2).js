...
function quickSettings(){
  ...
  // Check if currently active profile is a child
  var activeProfile=(_profiles||[]).find(function(p){return p.id===_activeProfileId;});
  var isChildActive = activeProfile && activeProfile.isParent===false;

  // If a child is logged in, only show their own settings — no parent access
  if(isChildActive){
    openSettings();
    return;
  }
  // Parent is active — if children exist and PIN is set, require PIN first
  if(children.length && D.parentPIN && !D.parentPinDisabled && !_parentDashUnlocked){
    _requirePin('Settings','Enter your parent PIN to access settings',(D.name||'P').charAt(0).toUpperCase(),function(){
      _parentDashUnlocked=true;
      openSettingsPicker();
    });
    return;
