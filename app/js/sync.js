...
function loadData(){
  ...
  for(const k of keys){
    try{
      ...
      // Also restore any keys in saved data not in DEF (e.g. pb, myInstruments added dynamically)
      for(const key of Object.keys(p)){ if(!(key in DEF) && typeof p[key]!=='function') D[key]=p[key]; }
...
async function cloudLoad(){
  ...
  try {
    ...
    // Also restore any keys in saved data not in DEF
    for(const k of Object.keys(saved)){ if(!(k in DEF) && typeof saved[k]!=='function') D[k] = saved[k]; }
