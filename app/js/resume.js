...
function _resumePageHtml(forPrint) {
  // Gather data directly from the form fields — no DOM scraping
  const v = id => { const e = document.getElementById(id); return e ? (e.value||'').trim() : ''; };
  const d = {
    name: v('rName'), title: v('rTitle'), email: v('rEmail'), phone: v('rPhone'),
    location: v('rLocation'), linkedin: v('rLinkedin'), website: v('rWebsite'),
    summary: v('rSummary'), skills: v('rSkills'), certs: v('rCerts'), langs: v('rLangs'),
    experience: collectEntries('exp'),
    education:  collectEntries('edu')
  };

  // Build sidebar and main content using same helpers as livePreview
  const sideHead = label =>
    '<div style="font-size:.58rem;font-weight:900;letter-spacing:2.5px;text-transform:uppercase;' +
    'color:#7dd3fc;border-bottom:1px solid rgba(255,255,255,.15);padding-bottom:.2rem;margin:.7rem 0 .35rem;">' + label + '</div>';
  ...
  const skillsList = d.skills ? d.skills.split(',').map(s => s.trim()).filter(Boolean)
    .map(s => '<span style="background:rgba(255,255,255,.12);border-radius:4px;padding:.1rem .35rem;font-size:.7rem;' +
              'display:inline-block;margin:0 .15rem .2rem 0;">' + escHtml(s) + '</span>').join('') : '';

  const expHtml = d.experience.length ? d.experience.map(e =>
    '<div style="margin-bottom:.65rem;">' +
      '<div style="display:flex;justify-content:space-between;align-items:baseline;">' +
        '<strong style="font-size:.85rem;">' + escHtml(e.title) + '</strong>' +
        '<span style="font-size:.7rem;color:#666;">' + escHtml(e.dates) + '</span>' +
      '</div>' +
      '<div style="font-size:.75rem;color:#1e3a5f;margin-bottom:.12rem;">' + escHtml(e.company) + (e.location ? ' &middot; ' + escHtml(e.location) : '') + '</div>' +
      (e.bullets ? '<div style="font-size:.73rem;color:#333;white-space:pre-line;">' + escHtml(e.bullets) + '</div>' : '') +
    '</div>'
  ).join('') : '';

  const eduHtml = d.education.length ? d.education.map(e =>
    '<div style="margin-bottom:.55rem;">' +
      '<div style="display:flex;justify-content:space-between;align-items:baseline;">' +
        '<strong style="font-size:.82rem;">' + escHtml(e.school) + '</strong>' +
        '<span style="font-size:.7rem;color:#666;">' + escHtml(e.dates) + '</span>' +
      '</div>' +
      '<div style="font-size:.73rem;color:#1e3a5f;">' + escHtml(e.degree) + (e.gpa ? ' &middot; ' + escHtml(e.gpa) : '') + '</div>' +
    '</div>'
  ).join('') : '';

  const sidebar =
    '<div style="font-size:1.2rem;font-weight:900;color:#fff;line-height:1.2;margin-bottom:.15rem;">' + (escHtml(d.name)||'Your Name') + '</div>' +
    (d.title ? '<div style="font-size:.72rem;color:#7dd3fc;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:.3rem;">' + escHtml(d.title) + '</div>' : '') +
    (contactItems ? sideHead('Contact') + '<div style="font-size:.7rem;color:#bfdbfe;">' + contactItems + '</div>' : '') +
    (d.skills ? sideHead('Skills') + skillsList : '') +
    (d.certs  ? sideHead('Certifications') + '<div style="font-size:.7rem;color:#bfdbfe;white-space:pre-line;">' + escHtml(d.certs) + '</div>' : '') +
    (d.langs  ? sideHead('Languages') + '<div style="font-size:.7rem;color:#bfdbfe;white-space:pre-line;">' + escHtml(d.langs) + '</div>' : '');

  const main =
    (d.summary ? mainHead('Profile') + '<div style="font-size:.78rem;color:#333;line-height:1.55;">' + escHtml(d.summary) + '</div>' : '') +
    (d.experience.length ? mainHead('Experience') + expHtml : '') +
    (d.education.length  ? mainHead('Education')  + eduHtml  : '');
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
    '<title>' + escHtml(d.name||'Resume') + '</title>' +
...
function previewResume() {
  const html = _resumePageHtml(false);
  if (!html) return;
...
// Wraps the edited preview content for printing
function _resumeEditedPageHtml(forPrint) {
  ...
  // Strip contenteditable attrs for clean output
  const cleanHtml = previewEl.innerHTML
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
    ...
    '<style>' +
      ...
      'html,body{font-family:Inter,Arial,sans-serif;background:#fff;}' +
      '@page{size:8.5in 11in portrait;margin:0;}' +
      'body > div{width:8.5in;min-height:11in;}' +
      '-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;' +
      previewCss +
    '</style></head><body>' +
    cleanHtml +
...
function printResume() {
  const html = _resumeDirty ? _resumeEditedPageHtml(true) : _resumePageHtml(true);
  if (!html) return;
  const win = window.open('', '_blank');
  win.document.write(html);
  win.document.close();
...
function _tmplModern(d) {
  ...
  const sidebar = `
    <div style="background:#1e3a5f;color:#e0f0ff;padding:1.6rem 1.1rem 1rem 1.1rem;min-height:100%;box-sizing:border-box;">
      <!-- Name & Title -->
...
function rFontSize(val) {
  if (!val) return;
  rFmt('fontSize', val);
}

function rSetColor(hex) {
  rFmt('foreColor', hex);
}

function rIndent(dir) {
  rFmt(dir > 0 ? 'indent' : 'outdent');
}

// Reset button — clears dirty flag and rebuilds from fields
function rResetFromFields() {
  const el = document.getElementById('resumePreview');
  if (!el) return;
  if (_resumeDirty) {
  ...
  const data = {
    ...
    location: v('rLocation'), linkedin: v('rLinkedin'), website: v('rWebsite'),
    summary: v('rSummary'), skills: v('rSkills'), certs: v('rCerts'), langs: v('rLangs'),
    experience: collectEntries('exp'), education: collectEntries('edu')
  };
  let html = '';
  if (tmpl === 'modern')  html = _tmplModern(data);
  else if (tmpl === 'classic') html = _tmplClassic(data);
  else if (tmpl === 'minimal') html = _tmplMinimal(data);
  else if (tmpl === 'bold')    html = _tmplBold(data);
  el.innerHTML = html;
  showToast('Preview reset from fields ↺');
}

// Override printResume and previewResume to use live preview HTML
// when in dirty/edited state (user's manual edits should print)
