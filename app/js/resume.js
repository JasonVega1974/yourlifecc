...
function _tmplModern(d) {
  const accent = '#1a56db';
  return `<div style="padding:1.5rem 1.8rem;font-family:'Inter',Arial,sans-serif;font-size:.82rem;line-height:1.5;color:#111;">
    <div style="text-align:center;margin-bottom:.8rem;">
      ${d.name ? `<div style="font-size:1.6rem;font-weight:900;color:#111;letter-spacing:-.5px;">${escHtml(d.name)}</div>` : '<div style="color:#bbb;font-size:1.1rem;">Your Name</div>'}
      ${d.title ? `<div style="font-size:.88rem;color:${accent};font-weight:600;margin:.12rem 0;">${escHtml(d.title)}</div>` : ''}
      <div style="font-size:.73rem;color:#555;margin-top:.25rem;">${escHtml(_contactLine(d.email,d.phone,d.location,d.linkedin,d.website))}</div>
    </div>
    ${d.summary ? _sectionHead('Summary', accent, accent) + `<div style="font-size:.8rem;color:#333;">${escHtml(d.summary)}</div>` : ''}
    ${d.experience.length ? _sectionHead('Experience', accent, accent) + _renderExp(d.experience, accent) : ''}
    ${d.education.length  ? _sectionHead('Education', accent, accent)  + _renderEdu(d.education, accent) : ''}
    ${d.skills ? _sectionHead('Skills', accent, accent) + `<div style="font-size:.78rem;color:#333;">${escHtml(d.skills)}</div>` : ''}
    ${d.certs  ? _sectionHead('Certifications', accent, accent) + `<div style="font-size:.78rem;color:#333;white-space:pre-line;">${escHtml(d.certs)}</div>` : ''}
    ${d.langs  ? _sectionHead('Languages', accent, accent) + `<div style="font-size:.78rem;color:#333;white-space:pre-line;">${escHtml(d.langs)}</div>` : ''}
  </div>`;
}
