...
function renderDailyPrompt(){ return;
  ...
  el.innerHTML = `
    <div style="background:rgba(34,211,238,.04);border:1px solid rgba(34,211,238,.1);border-radius:12px;padding:.7rem .9rem;margin-bottom:.6rem;cursor:pointer;" onclick="showSection('s-journal')">
      <div style="font-size:.55rem;text-transform:uppercase;letter-spacing:1.5px;color:#22d3ee;font-weight:700;margin-bottom:.25rem;">💭 TODAY'S JOURNAL PROMPT</div>
...
const EMOTION_LESSONS = [
  ...
  {icon:'🪞',title:'Self-Awareness',sub:'The foundation of every other emotional skill',color:'#22d3ee',
...
const LEADER_LESSONS = [
  ...
  {icon:'🗣️',title:'Influence Without Authority',sub:'Real leaders don\'t need a title',color:'#22d3ee',
...
// ══════════════════════════════════════════════════════════════
(function(){
  const COLORS = ['#38bdf8','#38bdf8','#f472b6','#34d399','#fbbf24','#fb923c','#22d3ee','#4ade80'];
...
// Fires a lightweight ping to user_activity table whenever a
// logged-in user navigates to a section. Used by admin dashboard
// to show session counts, last section visited, and engagement score.
// Silent fail — never breaks the app.
// ══════════════════════════════════════════════════════════════
(function(){
  const SUPA_URL  = 'https://hrohgwcbfgywkpnvqxhk.supabase.co';
  const SUPA_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imhyb2hnd2NiZmd5d2twbnZxeGhrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI4MjcxNDYsImV4cCI6MjA4ODQwMzE0Nn0.PuWtBpiMw2DiCLp26ZP_Rd9BwzFvWT0sNZrDUNdULyo';

  // Debounce: don't re-log the same section within 60 seconds
  let _lastKey = null;

  // Map section IDs (s-finance, s-hero, etc.) to human-readable names
  const SECTION_NAMES = {
    'hero':       'Dashboard',
    'finance':    'Finance',
    'school':     'School',
  ...
  window.trackSection = async function(sectionId) {
    try {
      // Only track logged-in users, skip demo mode
      if(typeof IS_DEMO !== 'undefined' && IS_DEMO) return;
      if(typeof _supaUser === 'undefined' || !_supaUser) return;

      // Resolve human name — strip leading 's-' prefix if present
      const key = sectionId.replace(/^s-/, '');
      const sectionName = SECTION_NAMES[key] || key;

      // Debounce: skip if same section within last 60 seconds
      const dedupeKey = sectionName + '|' + Math.floor(Date.now() / 60000);
      if(_lastKey === dedupeKey) return;
      _lastKey = dedupeKey;
      ...
      await fetch(`${SUPA_URL}/rest/v1/user_activity`, {
        method: 'POST',
        headers: {
          'apikey': SUPA_ANON,
...
function renderTypingBests(){
  ...
  el.innerHTML = diffs.map(d=>{
    const b = bests[d];
    return `<div class="fst" style="text-align:center;border-top:3px solid ${d==='easy'?'#86efac':d==='medium'?'#7dd3fc':'#f472b6'};">
...
function saveProfile(){
  ...
  // Also update the parent profile name in _profiles so the welcome
  // card always reflects the correct name and it syncs to Supabase.
  if(typeof _profiles !== 'undefined' && typeof _activeProfileId !== 'undefined'){
    const activeProf = _profiles.find(p => p.id === _activeProfileId);
    // Only update name on the parent profile, never on a child profile
    if(activeProf && activeProf.isParent !== false){
...
<h4 style="color:#22d3ee;margin:.8rem 0 .3rem;">The identity trap</h4>
...
<h4 style="color:#22d3ee;margin:.8rem 0 .3rem;">A more stable foundation</h4>
...
<h4 style="color:#22d3ee;margin:.8rem 0 .3rem;">Practical identity building</h4>
<ul>
...
<h4 style="color:#22d3ee;margin:.8rem 0 .3rem;">The comparison problem</h4>
...
<div style="margin-top:1rem;padding:.85rem;border-radius:11px;background:rgba(8,145,178,.1);border:1px solid rgba(8,145,178,.25);">
  <strong style="color:#38bdf8;">The truth:</strong> Being a man isn't about suppressing who you are — it's about becoming who you were made to be. Strong enough to lead, humble enough to serve, brave enough to feel, and wise enough to ask for help.
</div>
    `
  },
  {
...
<p><b style="color:#22d3ee;">💼 Straight to Work:</b> Nothing wrong with starting to work immediately. Many successful people never went to college. The key: pick a field with growth potential, not just whatever pays this week. Learn skills on the job, take free online courses, and always be building toward something — not just clocking hours.</p>
...
,
  {id:'world-religions',icon:'🕊️',title:'Freedom of Religion — Understanding What Others Believe',sub:'Respecting all faiths while knowing your own',color:'#22d3ee',showFor:[],body:`<p><strong>The United States was founded on freedom of religion. This means every person has the right to believe — or not believe — according to their own conscience.</strong> Understanding what others believe does not weaken your faith. It strengthens your ability to respect, communicate with, and love people who see the world differently.</p>
