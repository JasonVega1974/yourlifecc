...
function openSettings(){
  ...
  // Inject "Parent Settings" button into child view so they can switch without refresh
  var _pab=document.getElementById('sp-parent-access-btn');
  if(_spIsChild){
    if(!_pab){
      _pab=document.createElement('div');
      _pab.id='sp-parent-access-btn';
      _pab.style.cssText='padding:.6rem 1rem 0;margin-bottom:.2rem;';
      _pab.innerHTML='<button style="width:100%;padding:.45rem;background:rgba(255,255,255,.05);color:var(--tx2);border:1px solid rgba(255,255,255,.1);border-radius:9px;font-size:.73rem;font-weight:700;cursor:pointer;font-family:var(--fn);" onclick="openParentFromChildSettings()">🔒 Switch to Parent Settings</button>';
      var _spFirst=document.querySelector('#sp-pb');
      if(_spFirst && _spFirst.parentNode) _spFirst.parentNode.insertBefore(_pab,_spFirst);
    }
    _pab.style.display='';
  } else {
...
function openParentFromChildSettings(){
  if(!D.parentPIN){
    // No PIN set — switch directly to parent view
    var _pp=(_profiles||[]).find(function(p){return p.isParent!==false;});
    if(_pp){ _activeProfileId=_pp.id; localStorage.setItem('ylcc_active_profile',_activeProfileId||''); }
    openSettings(); return;
  }
  var _entered=prompt('Enter Parent PIN:');
  if(_entered===null) return;
  if(String(_entered).trim()===String(D.parentPIN)){
    var _pp=(_profiles||[]).find(function(p){return p.isParent!==false;});
    if(_pp){ _activeProfileId=_pp.id; localStorage.setItem('ylcc_active_profile',_activeProfileId||''); }
    openSettings();
  } else {
...
function showSection(id, fromMobile){
  ...
  if(id==='s-hero') setTimeout(()=>{ renderGettingStarted(); renderPathway(); renderMonthlyChallenge(); renderDailyPrompt(); renderBadges(); updateDashCards(); renderHeroScreenTime(); renderDevMap(); },50);
  ...
  if(id==='s-scripture')  setTimeout(()=>{ typeof initScripture==='function'&&initScripture(); typeof renderScripturePage==='function'&&renderScripturePage(); typeof renderDevotionals==='function'&&renderDevotionals(); typeof renderBibleReadings==='function'&&renderBibleReadings(); typeof renderLearnBibleGrid==='function'&&renderLearnBibleGrid(); },80);
...
// ── SCHEDULE BLOCK PICKER ─────────────────────────────────────
// Adds a quick "Add Time Block" panel above the schedule grid
// that lets users pick day, start time, end time, label, and color

function initScheduleBlockPicker(){
  var container = document.getElementById('schedWrap');
  if(!container) return;
  // Only inject once
  if(document.getElementById('schedBlockPicker')) return;
  var section = document.getElementById('s-schedule');
  if(!section) return;
  var card = section.querySelector('.card');
  if(!card) return;

  var picker = document.createElement('div');
  picker.id = 'schedBlockPicker';
  picker.style.cssText = 'background:rgba(255,255,255,.03);border:1px solid rgba(56,189,248,.15);border-radius:12px;padding:.8rem 1rem;margin-bottom:.8rem;';
  picker.innerHTML = [
    '<div style="font-size:.72rem;font-weight:800;color:var(--c);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:.6rem;">➕ Add Time Block</div>',
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem;margin-bottom:.4rem;">',
    '  <div>',
    '    <label style="font-size:.65rem;color:var(--tx2);">Day</label>',
    '    <select id="sbDay" style="width:100%;font-size:.78rem;border-radius:8px;padding:.35rem .5rem;background:var(--input-bg,rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.1);color:var(--tx);">',
    ...
    '  </div>',
    '  <div>',
    '    <label style="font-size:.65rem;color:var(--tx2);">Label</label>',
    '    <input id="sbLabel" type="text" placeholder="e.g. Math Study" style="width:100%;font-size:.78rem;border-radius:8px;padding:.35rem .5rem;background:var(--input-bg,rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.1);color:var(--tx);box-sizing:border-box;">',
    '  </div>',
    '</div>',
    '<div style="display:grid;grid-template-columns:1fr 1fr 80px;gap:.4rem;margin-bottom:.6rem;">',
    '  <div>',
    '    <label style="font-size:.65rem;color:var(--tx2);">Start Time</label>',
    '    <input id="sbStart" type="time" value="08:00" style="width:100%;font-size:.78rem;border-radius:8px;padding:.35rem .5rem;background:var(--input-bg,rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.1);color:var(--tx);box-sizing:border-box;">',
    '  </div>',
    '  <div>',
    '    <label style="font-size:.65rem;color:var(--tx2);">End Time</label>',
    '    <input id="sbEnd" type="time" value="09:00" style="width:100%;font-size:.78rem;border-radius:8px;padding:.35rem .5rem;background:var(--input-bg,rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.1);color:var(--tx);box-sizing:border-box;">',
    '  </div>',
    '  <div>',
    '    <label style="font-size:.65rem;color:var(--tx2);">Color</label>',
    '    <select id="sbColor" style="width:100%;font-size:.72rem;border-radius:8px;padding:.35rem .3rem;background:var(--input-bg,rgba(255,255,255,.06));border:1px solid rgba(255,255,255,.1);color:var(--tx);">',
    '      <option value="#38bdf8">🔵 Blue</option><option value="#a78bfa">🟣 Purple</option>',
    '      <option value="#22c55e">🟢 Green</option><option value="#fbbf24">🟡 Yellow</option>',
    '      <option value="#f87171">🔴 Red</option><option value="#fb923c">🟠 Orange</option>',
    '      <option value="#f472b6">🩷 Pink</option><option value="#94a3b8">⬜ Gray</option>',
    '    </select>',
    '  </div>',
    '</div>',
    '<div style="display:flex;gap:.4rem;">',
    '  <button class="btn bp" style="flex:1;font-size:.78rem;" onclick="addScheduleBlock()">+ Add Block</button>',
    '  <button class="btn bgh bs" style="font-size:.78rem;" onclick="renderScheduleBlocks()">↺ Refresh</button>',
    '</div>',
    '<div id="schedBlockList" style="margin-top:.6rem;max-height:160px;overflow-y:auto;"></div>'
  ].join('');
...
function addScheduleBlock(){
  var day   = (document.getElementById('sbDay')||{}).value||'Mon';
  var label = ((document.getElementById('sbLabel')||{}).value||'').trim();
  var start = (document.getElementById('sbStart')||{}).value||'08:00';
  var end   = (document.getElementById('sbEnd')||{}).value||'09:00';
  var color = (document.getElementById('sbColor')||{}).value||'#38bdf8';
  if(!label){ showToast('Enter a label for this block'); return; }
  if(start >= end){ showToast('End time must be after start time'); return; }
  if(!D.scheduleBlocks) D.scheduleBlocks = [];
  D.scheduleBlocks.push({ id:Date.now(), day:day, label:label, start:start, end:end, color:color });
  save();
  renderScheduleBlocks();
  // Clear label field for next entry
  var lbl = document.getElementById('sbLabel'); if(lbl) lbl.value='';
  showToast('Block added! 📅');
}

function deleteScheduleBlock(id){
  D.scheduleBlocks = (D.scheduleBlocks||[]).filter(function(b){ return b.id!==id; });
  save();
  renderScheduleBlocks();
}

function renderScheduleBlocks(){
  var el = document.getElementById('schedBlockList'); if(!el) return;
  var blocks = D.scheduleBlocks||[];
  if(!blocks.length){ el.innerHTML='<div style="font-size:.72rem;color:var(--tx2);text-align:center;padding:.5rem;">No blocks added yet.</div>'; return; }
  var dayOrder = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
  var sorted = blocks.slice().sort(function(a,b){
    var di = dayOrder.indexOf(a.day) - dayOrder.indexOf(b.day);
    return di!==0 ? di : a.start.localeCompare(b.start);
  });
  el.innerHTML = sorted.map(function(b){
    return '<div style="display:flex;align-items:center;gap:.5rem;padding:.3rem .4rem;border-radius:8px;background:rgba(255,255,255,.03);border-left:3px solid '+b.color+';margin-bottom:.3rem;">'
      +'<div style="flex:1;">'
      +'<div style="font-size:.78rem;font-weight:700;color:var(--tx);">'+b.label+'</div>'
      +'<div style="font-size:.65rem;color:var(--tx2);">'+b.day+' · '+b.start+'–'+b.end+'</div>'
      +'</div>'
      +'<button onclick="deleteScheduleBlock('+b.id+')" style="background:none;border:none;color:var(--tx2);cursor:pointer;font-size:.9rem;padding:.1rem .3rem;">✕</button>'
      +'</div>';
  }).join('');
}

// Auto-inject block picker when schedule section opens
(function(){
  var _origBuildSchedule = typeof buildSchedule === 'function' ? buildSchedule : null;
  window.buildSchedule = function(){
    if(_origBuildSchedule) _origBuildSchedule.apply(this, arguments);
    setTimeout(initScheduleBlockPicker, 100);
  };
})();

// ── MUSIC & PRACTICE — ensure functions exist ─────────────────
// Fallback implementations in case the primary file fails to define them

if(typeof addMyInstrument === 'undefined'){
window.addMyInstrument = function(){
  var sel = document.getElementById('addInstrument');
  var goalEl = document.getElementById('practiceGoalMin');
  var name = sel ? sel.value : '';
  if(!name){ showToast('Select an instrument first'); return; }
  if(!D.myInstruments) D.myInstruments = [];
  if(D.myInstruments.find(function(i){ return i.name===name; })){ showToast('Already added!'); return; }
  D.myInstruments.push({ id:Date.now(), name:name, goalMin:parseInt(goalEl&&goalEl.value||60), totalMin:0 });
  if(sel) sel.value='';
...
if(typeof renderMyInstruments === 'undefined'){
window.renderMyInstruments = function(){
  var el = document.getElementById('myInstrumentsList'); if(!el) return;
  var instruments = D.myInstruments||[];
  if(!instruments.length){ el.innerHTML='<div style="font-size:.75rem;color:var(--tx2);text-align:center;padding:.5rem;">No instruments added yet.</div>'; return; }
  el.innerHTML = instruments.map(function(inst){
    var pct = Math.min(100, Math.round(((inst.totalMin||0)/((inst.goalMin||60)*4))*100));
    return '<div style="display:flex;align-items:center;gap:.5rem;padding:.4rem .5rem;background:rgba(255,255,255,.03);border-radius:8px;margin-bottom:.3rem;">'
      +'<div style="flex:1;">'
      +'<div style="font-size:.82rem;font-weight:800;color:var(--tx);">'+inst.name+'</div>'
      +'<div style="font-size:.65rem;color:var(--tx2);">Goal: '+(inst.goalMin||60)+' min/wk · Total: '+(inst.totalMin||0)+' min</div>'
      +'<div style="height:4px;background:rgba(255,255,255,.08);border-radius:2px;margin-top:.3rem;"><div style="height:100%;width:'+pct+'%;background:var(--c);border-radius:2px;transition:width .4s;"></div></div>'
      +'</div>'
      +'<button onclick="removeMyInstrument('+inst.id+')" style="background:none;border:none;color:var(--tx2);cursor:pointer;font-size:.85rem;">✕</button>'
      +'</div>';
  }).join('');
};}

if(typeof removeMyInstrument === 'undefined'){
window.removeMyInstrument = function(id){
  D.myInstruments = (D.myInstruments||[]).filter(function(i){ return i.id!==id; });
  save(); renderMyInstruments(); updatePracticeSelects();
};}

if(typeof updatePracticeSelects === 'undefined'){
window.updatePracticeSelects = function(){
  var selects = ['practiceInstrSelect','gigInstr'];
  selects.forEach(function(sid){
    ...
    (D.myInstruments||[]).forEach(function(inst){
...
var _practiceInterval = null;
var _practiceSecs = 0;
var _practiceTarget = 0;
var _practiceRunning = false;

if(typeof setPracticeTimer === 'undefined'){
window.setPracticeTimer = function(mins){
  _practiceTarget = mins * 60;
  _practiceSecs = mins * 60;
  _practiceRunning = false;
  clearInterval(_practiceInterval);
  var disp = document.getElementById('practiceTimerDisp');
  if(disp) disp.textContent = _fmtPracticeTime(_practiceSecs);
  var btn = document.getElementById('practiceStartBtn');
  if(btn) btn.textContent = '▶ Start';
};}

if(typeof startPractice === 'undefined'){
window.startPractice = function(){
  ...
  if(_practiceSecs <= 0){ showToast('Set a timer first (15m, 30m, etc.)'); return; }
  _practiceRunning = true;
  var btn = document.getElementById('practiceStartBtn');
  if(btn) btn.textContent = '⏸ Pause';
  _practiceInterval = setInterval(function(){
    if(_practiceSecs <= 0){
      clearInterval(_practiceInterval);
      _practiceRunning = false;
      var b = document.getElementById('practiceStartBtn'); if(b) b.textContent = '▶ Start';
...
if(typeof stopPractice === 'undefined'){
window.stopPractice = function(){
  clearInterval(_practiceInterval);
  _practiceRunning = false;
  var elapsed = _practiceTarget - _practiceSecs;
...
function _fmtPracticeTime(secs){
  var m = Math.floor(secs/60), s = secs%60;
  return (m<10?'0':'')+m+':'+(s<10?'0':'')+s;
}

function _savePracticeSession(elapsedSecs){
  var mins = Math.round((elapsedSecs !== undefined ? elapsedSecs : _practiceTarget) / 60);
  if(mins < 1) return;
  var instrSel = document.getElementById('practiceInstrSelect');
  var instrName = instrSel ? instrSel.value : '';
  if(!D.practiceSessions) D.practiceSessions = [];
  D.practiceSessions.push({ id:Date.now(), instrument:instrName, mins:mins, date:new Date().toISOString().slice(0,10) });
  // Update totalMin on instrument
  if(instrName && D.myInstruments){
    var inst = D.myInstruments.find(function(i){ return i.name===instrName; });
...
if(typeof renderPracticeWeek === 'undefined'){
window.renderPracticeWeek = function(){
  var el = document.getElementById('practiceWeekGrid'); if(!el) return;
  var today = new Date();
  var days = [];
  for(var i=6; i>=0; i--){
  ...
  var sessions = D.practiceSessions||[];
  el.innerHTML = '<div style="display:flex;gap:.3rem;justify-content:space-between;">'
    + days.map(function(ds){
      var dayMins = sessions.filter(function(s){ return s.date===ds; }).reduce(function(acc,s){ return acc+s.mins; },0);
      var h = Math.min(60, dayMins);
      var label = ['S','M','T','W','T','F','S'][new Date(ds+'T12:00:00').getDay()];
      return '<div style="flex:1;text-align:center;">'
        +'<div style="height:'+Math.max(4,h)+'px;background:'+(dayMins?'var(--c)':'rgba(255,255,255,.08)')+';border-radius:3px;margin-bottom:.2rem;transition:height .3s;"></div>'
...
if(typeof logGig === 'undefined'){
window.logGig = function(){
  var instr = (document.getElementById('gigInstr')||{}).value||'';
  var title = ((document.getElementById('gigTitle')||{}).value||'').trim();
  var date  = (document.getElementById('gigDate')||{}).value||new Date().toISOString().slice(0,10);
  var mins  = parseInt((document.getElementById('gigMins')||{}).value||0);
  var notes = ((document.getElementById('gigNotes')||{}).value||'').trim();
  if(!title){ showToast('Enter a session title'); return; }
  if(!D.gigLog) D.gigLog=[];
  D.gigLog.unshift({ id:Date.now(), instrument:instr, title:title, date:date, mins:mins||0, notes:notes });
  // Clear fields
  ['gigTitle','gigNotes','gigMins'].forEach(function(id){ var e=document.getElementById(id); if(e) e.value=''; });
  // Update totalMin on instrument
  if(instr && mins && D.myInstruments){
    var inst = D.myInstruments.find(function(i){ return i.name===instr; });
    if(inst) inst.totalMin = (inst.totalMin||0) + mins;
  }
  save();
  renderGigLog();
...
if(typeof renderGigLog === 'undefined'){
window.renderGigLog = function(){
  var el = document.getElementById('gigLog'); if(!el) return;
  var log = D.gigLog||[];
  ...
  el.innerHTML = log.map(function(g){
    return '<div style="padding:.5rem .6rem;border-bottom:1px solid rgba(255,255,255,.04);">'
      +'<div style="display:flex;align-items:center;gap:.4rem;margin-bottom:.15rem;">'
      +'<span style="font-size:.78rem;font-weight:800;color:var(--tx);">'+g.title+'</span>'
      +(g.mins?'<span style="font-size:.65rem;color:var(--c);">'+g.mins+'min</span>':'')
      +'</div>'
      +'<div style="font-size:.65rem;color:var(--tx2);">'+(g.instrument?g.instrument+' · ':'')+g.date+'</div>'
      +(g.notes?'<div style="font-size:.7rem;color:var(--tx2);margin-top:.15rem;">'+g.notes+'</div>':'')
      +'</div>';
  }).join('');
};}

if(typeof buildCraftSection === 'undefined'){
...
if(typeof initMusicAndSports === 'undefined'){
window.initMusicAndSports = function(){
  buildCraftSection();
  // Set today's date in gig date field
  var gd = document.getElementById('gigDate');
